...
ui <- fluidPage(# Application title
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      textInput(
        inputId = "CTRLcols",
        label = "Columns containing controls (comma delimited)",
...
server <- function(input, output) {
  observeEvent(input$redraw, {
    if (!is.null(input$file1)) {
      ...
      # Cols containing CTRLs
      CTRLcols <- c(unlist(strsplit(input$CTRLcols, ",")))
      ...
      #### Checkpoint CTRL group not empty #####
      if (!(sum(XTTclean$Harvest == 0) <= 0)) {
        ...
        for (lvl in lvls) {
          ...
          while (outliers < maxoutliers) {
            outliers <- outliers + 1
            subref <-
              reflist[reflist$Harvest == lvl &
                        !(reflist$id %in% idlist), ]
        ...
        suppressWarnings(# suppress warning because of uninitialised column: 'FoldChange'
          for (i in 1:nrow(XTTout)) {
            XTTout$FoldChange[i] <-
              XTTout$Data[i] / as.numeric(avg$Avg[avg$Harvest == 0 &
        ...
        output$downloadPlot <-
          downloadHandler(
            ...
            filename = function() {
              paste(input$file1$datapath, '.png', sep = '')
            },
        ...
        output$Warning <-
          renderText("Cant update Plot: CTRL group is empty! Check exluded columns and CTRL columns.")
      }
    } else {
      output$Warning <-
        renderText("Please upload a .xlsx file with XTT data!")
    }
